...
export default function Builder() {
  ...
  const handleDownloadPDF = () => {
    ...
    // Helper to remove any invisible full-screen overlays left by html2pdf
    const cleanupOverlays = () => {
      try {
        ...
        // remove any full-screen fixed overlays that are nearly transparent
        const els = Array.from(document.body.children);
        els.forEach((el) => {
          const cs = window.getComputedStyle(el);
          if (cs.position === 'fixed') {
            const coversViewport = cs.top === '0px' && cs.left === '0px' && cs.right === '0px' && (cs.bottom === '0px' || cs.height === '100%');
            const opacity = parseFloat(cs.opacity || '1');
            const zIndex = parseInt(cs.zIndex || '0', 10);
            if (coversViewport && opacity < 0.05 && zIndex >= 999) {
              el.remove();
            }
          }
        });
    ...
    // Run html2pdf and ensure cleanup runs after completion/errors
    try {
